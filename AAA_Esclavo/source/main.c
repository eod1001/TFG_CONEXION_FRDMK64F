...
void uart_Task(void *arg)  //comando-> uart/Mode/0x01/ || uart/Vel1/0xff/ || uart/GVel1/
{
	...
	for(;;){
		...
		if (xReceivedBytes)
		{
			...
			var = token;
			PRINTF("\n %s, %s, %s ",cmd,func,var);
			PRINTF("\n RECIBO uart: %s ",buffer);
			PRINTF("\n RESULTADO: VAR: %s, FUNC: %s",var, func);
			...
			if(strcmp(func,"Mode")==0){
				...
				PRINTF("\nLLEGUE2");
				PRINTF("\n SE ENVIO LA VELOCIDAD 1");

				//WRITER
				char* config_msg = "g_vel/mot1/";
				int intg_vel1 = (int)g_vel1;
				char * charg_vel1 = (char *)intg_vel1;
				strcat(config_msg, charg_vel1);
				PRINTF("LLEGUE3");
				strcat(config_msg, "/");
				PRINTF("LLEGUE4");
				...
				//WRITER
				char* config_msg = "g_vel/mot2/";
				strcat(config_msg, g_vel2);
				struct netconn * netconn;
				IP4_ADDR(&ipaddr, 192,168,0,16);
				netconn = netconn_new(NETCONN_TCP);
				vTaskDelay(2500/portTICK_RATE_MS);
