 ...
 *
 * @brief
 * This software is the first "Master" part of a pilot plant consisting of 3
 * FRDM K64F boards.  This board should be upgraded by using the arduino basic
 * I/O expansion board. From this board, the user can communicate with the
 * other two boards via network cable and TCP/IP protocols. By pressing the
 * buttons and potentiometers on the master board and the shield will send
 * commands to the other two boards. The other two Embedded Systems will
 * communicate with two emg30 motors and an LCD display. For more information
 * check the documentation of the site:
 * https://github.com/eod1001/TFG_CONEXION_FRDMK64F
 *
...
/**
 * @brief Obtains the voltage value of the temperature sensor.
 *
 * This task obtains the voltage value that passes
 * through the temperature sensor of the shield board.
 * After this, a command is created with this data and
 * sent to the slave board "A".
 *
...
/**
 * @brief Turn off the shield leds.
 *
 */
void turn_off_expansion_leds(){
...
/**
 * @brief It generates and sends, to the corresponding boards,
 * 			the commands required by the user.
 *
 * Depending on the regulation of the potentiometers and pressing
 * of the shield buttons by the user, it generates commands that
 * will be sent to the corresponding slave board.
 *
 * @param[in] arg argument to the thread.
 */
void ADC_Writer_thread(void * arg)
 ...
 *
 * @param[cmd] First part of the command received, indicates the command type.
 * @param[nMot] Indicates the motor to which the command refers.
 * @param[vel] This is the speed at which the motor rotates.
 */
void rx_command_check(char * cmd, char * nMot, char * vel)
{
	char * nMotor;
...
void init_IP(void * arg)
{
	...
	for(;;)
	{
		...
		/* The suspended tasks are started and the current task is deleted. */
