...
#define UART_COMMAND "uat:"
#define UART_COMMAND_INDEX 4 /**< Index of the color argument. */
...
void tcp_writer_thread(void * arg);
...
MessageBufferHandle_t uart_handler; /**< Buffer handler of uart_Task. */
...
TaskHandle_t tcp_writer_handle; /**< Task handle of tcp_listener_thread. */
...
int main(void)
{
	...
	if (xTaskCreate(tcp_writer_thread, "tcp_writer_thread",
			DEFAULT_THREAD_STACKSIZE, NULL, DEFAULT_THREAD_PRIO, &tcp_writer_handle) != pdPASS)
	{
		PRINTF("tcp_writer_thread creation failed.\n");
	}

	/* Task suspended until the TCP/IP stack is initialized. */
		vTaskSuspend(tcp_writer_handle);
...
void uart_Task(void *arg)  //comando-> uat:/Mode/0x01   ||   uat:/Vel1/0xff
{
	...
	for (;;)
	{
		...
		if (xReceivedBytes)
		{
			...
			PRINTF("RECIBO uart: %s \n",buffer);
			token = strtok(NULL, delimitador);
			func = token;
			token = strtok(NULL, delimitador);
			var = token;

			PRINTF("RESULTADO: VAR: %s, FUNC: %s",var, func);
			...
			if(strcmp(func,"Mode")==0){
				...
				UART_WriteBlocking(UART4, &var,sizeof(var));
				PRINTF("SE ENVIO LA ACELERACION \n");
...
void tcp_writer_thread(void * arg)
{
	...
	for (;;)
	{
		...
		if (1){
			...
			netconn_write(netconn, string,strlen(string), TCP_WRITE_FLAG_COPY);
		}
...
void rx_command_check(char * buffer, uint16_t null_terminator_pos)
{
	...
	else
	{
		...
		else if (strncmp(buffer, UART_COMMAND, COMMAND_SIZE) == 0)
		{
			vTaskResume(uart_Task);
